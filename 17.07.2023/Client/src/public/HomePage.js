...
import { sendLetterToAnotherFile } from './ContentPage.js';
...
function HomePage() {
  ...
  const handleSignupCompletion = (letter) => {
    setIsSignedUp(true);
    setSignupLetter(letter)
    sendLetterToAnotherFile(letter)
  }
      ...
      {loading ? (
        ...
        <>
          <nav className="hp-nav">
            ...
            <div>
              {isSignedUp ? (
                <button className="nav-aftr-btn">{signupLetter}</button>
              ) : (
          ...
          {showSignup && <SignUp toggleLogin={toggleLogin} close={close} handleSignupCompletion={handleSignupCompletion} />}
...
function SignUp({ toggleLogin, close, handleSignupCompletion }) {
      ...
      fetch("http://localhost:100/signup", {
        ...
        if (datas.sign_msg) {
          ...
          if (datas.message) {
            ...
            handleSignupCompletion(name[0].toUpperCase());
            close();
          } else {
