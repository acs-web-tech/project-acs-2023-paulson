...
const PostBar = () => {
  ...
  if (ch === -1) {
    ...
    return (
      <div className="cp-post-page-con">
        {a.map((title, index) => (
          <PostHolder key={index} title={title} handleDeleteButton={handleDeleteButton} />
        ))}
      </div>
    );
  }
...
const PostHolder = ({ title, handleDeleteButton }) => {
...
function PageBar() {
  ...
  if (ch === -1) {
    ...
    return (
      <div>
        {a.map((title, index) => (
          <PageHolder key={index} title={title} />
        ))}
      </div>
    )
  }
...
function PageHolder({ title }) {

  const openPage = () => {
    fetch('http://localhost:100/showLink', {
      method: 'POST',
      ...
      if (datas.link.length !== 0) {
        window.location.href = datas.link[0].link;
      }
    })
  }
...
// Function for Themes used by Published Page
